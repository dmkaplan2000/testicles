...
% Local package for dealing with affiliations
\usepackage{mylocalmacropackage}
...
$if(address)$
$for(address)$
\Newlabel{$address.code$}{$address.address$}
$endfor$
$endif$
...
\address{\Ref{$authors.affiliation$}}
$endfor$
$endif$
$if(authors.email)$\email{\href{mailto:$authors.email$}{$authors.email$}}$endif$$if(authors.thanks)$\thanks{$authors.thanks$}$else$$if(authors.corresponding_author)$\thanks{Corresponding author; Email: \href{mailto:$authors.email$}{$authors.email$}}$endif$$endif$
$sep$\and
$endfor$
}
